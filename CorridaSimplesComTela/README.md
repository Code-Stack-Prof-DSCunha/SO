...
* Aplicação em JAVA Sequencial sem concorrência (sem Threads) com Interface Gráfica.
* O objetivo é manter a lógica sequencial (sem Threads), o código se torna um clássico aplicativo de console (CLI). 
* A tela de saídas em Janela (ambiente gráfico com AWT), o que torna o código ainda enxuto e ideal para visualizar uma corrida em fluxo.
...
```bash
javac CorridaSaposSequencial.java
```
*Isso gerará um arquivo chamado `CorridaSaposSequencial.class`.*

### 3. Executar
Após a compilação, você pode rodar o programa com o comando `java`:

```bash
java CorridaSaposSequencial
```
